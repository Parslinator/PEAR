...
if run_full_optimization == 'y':
    ...
    def load_adjustments():
        adjustments = {}
        try:
            with open(running_tracker_file, 'r') as file:
                lines = file.readlines()  # Read all lines in the file
                if not lines:  # Check if the file is empty
                    print("No running adjustments found. Starting a new tracker.")
                    return adjustments  # Return an empty dictionary if the file is empty
                for line in lines:
                    team, adjustment = line.strip().split(", ")  # Split each line into team and adjustment
                    team_name = team.split(": ")[1]  # Extract the team name
                    adjustment_amount = int(adjustment.split(": ")[1])  # Extract and convert the adjustment amount
                    adjustments[team_name] = adjustment_amount  # Store in the dictionary
        except FileNotFoundError:
            print("No running adjustments file found. Starting a new tracker.")
        return adjustments  # Return the dictionary of adjustments


    
    def save_adjustments(adjustments):
        with open(running_tracker_file, 'w') as file:
            for team, adjustment in adjustments.items():
                file.write(f"Team: {team}, Adjustment: {adjustment}\n")

    # Save weekly changes to a separate file
    def save_weekly_changes(adjustments_this_week):
        with open(weekly_changes_file, 'a') as file:
            for team, adjustment in adjustments_this_week:
                file.write(f"Team: {team}, Adjustment: {adjustment}\n")

    
    while True:
        new_team_adjustment = input("Do you want a new team adjustment? (y/n): ").strip().lower()
        if new_team_adjustment == 'y':
            adjustments = load_adjustments()
            adjustments_this_week = []
            print("If you've already ran the full adjustment tracker, select yes.")
            print("If you have not run tracker, select no")
            add_now = input("Make change now? (y/n): ")
            team_name = input("Enter team name: ").strip()
            if team_name in team_data['team'].values:
                while True:
                    try:
                        adjustment_amount = int(input(f"Enter adjustment amount for {team_name}: "))
                        if add_now == 'y':
                            team_data.loc[team_data['team'] == team_name, 'power_rating'] += adjustment_amount
                        if team_name in adjustments:
                            adjustments[team_name] += adjustment_amount
                        else:
                            adjustments[team_name] = adjustment_amount
                        adjustments_this_week.append((team_name, adjustment_amount))
                        break
                    except ValueError:
                        print("Please enter a valid integer for the adjustment amount.")
            save_adjustments(adjustments)
            save_weekly_changes(adjustments_this_week)
        else:
            last_week_update = input("Do you want to update using the running tracker? (y/n): ").strip().lower()
            if last_week_update == 'y':
                try:
                    with open(running_tracker_file, "r") as file:
                        for line in file:
                            team, adjustment = line.strip().split(", ")
                            team_name = team.split(": ")[1]
                            adjustment_amount  = int(adjustment.split(": ")[1])
                            team_data.loc[team_data['team'] == team_name, 'power_rating'] += adjustment_amount
                    break
                except FileNotFoundError:
