...
def all_136_teams(all_data, column, ascending_direction, team_logos, digits, current_week, title, folder_path, file_path, subtitle=None):
    all_data = all_data.sort_values(column, ascending=ascending_direction).reset_index(drop=True)
    ...
    fig.patch.set_facecolor('#CECEB2')
    plt.suptitle(f"{title}", fontsize=20, y=0.905, x=0.52, fontweight='bold')
    min_rating = all_data[column].min()
    max_rating = all_data[column].max()
