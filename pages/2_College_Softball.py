...
schedule_df = pd.read_csv(f"./PEAR/PEAR Softball/y{current_season}/schedule_{current_season}.csv")
...
stats_page = f"{base_url}/stats/softball/d1"
def get_soup(url):
...
st.title(f"{current_season} Softball PEAR")
...
with col2:
    ...
    with st.container(border=True, height=440):
        st.dataframe(modeling_stats_copy[['WAR', 'WPOE', 'PYTHAG', 'ERA', 'WHIP', 'RPG', 'BA', 'OBP', 'SLG', 'OPS', 'Conference']], width='stretch')
