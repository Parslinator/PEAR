...
if os.path.exists(folder_path):
    ...
    if png_files:
        if postseason == True:
            st.markdown(f'<h2 id="cfb-game-previews-for-bowl-games">CFB Game Previews for Bowl Games</h2>', unsafe_allow_html=True)
        else:
            st.markdown(f'<h2 id="cfb-game-previews-for-week-{current_week}">CFB Game Previews for Week {current_week}</h2>', unsafe_allow_html=True)
