...
folder_path = f"./PEAR/PEAR Softball/y{current_season}"
...
soup = get_soup(f"{base_url}/stats/softball/d1")
...
projected_rpi = scrape_warrennolan_table(
    'https://www.warrennolan.com/softball/2025/rpi-predict',
...
STAT_TRANSFORMS = {
    ...
    "Team Strikeouts Per Seven Innings": lambda df: df.rename(columns={"K/7": "KP7"}).drop(columns=['Rank', 'G', 'IP', 'SO']),

    "Total Runs": lambda df: df.assign(
        RPG=df["R"] / df["G"]
    ).rename(columns={"R": "RS"}).drop(columns=['Rank', 'G']),

    "WHIP": lambda df: df.drop(columns=['Rank', 'G', 'HA', 'IP']),
}
...
softball_stats = clean_and_merge(raw_stats, STAT_TRANSFORMS)
softball_stats = pd.merge(softball_stats, elo_data, on='Team', how='left')

missing_teams = set(elo_data['Team']) - set(softball_stats['Team'])
...
STAT_TRANSFORMS_WOBA = {
    "Hit By Pitch": lambda df: df[["Team", "HBP"]].drop_duplicates(subset="Team"),
    
    "Hits": lambda df: df[["Team", "H"]],
    
    "Doubles per Game": lambda df: df[["Team", "2B"]].drop_duplicates(subset="Team"),

    "On Base Percentage": lambda df: df[["Team", "AB"]],
    
    "Triples": lambda df: df[["Team", "3B"]],
    
    "Home Runs": lambda df: df.assign(HRPG=df['HR']/df['G']).drop(columns=["Rank", "G"]),
    
    "Total Sacrifice Flies": lambda df: df[["Team", "SF"]],
    
    "Total Runs": lambda df: df.assign(
        RPG=df["R"] / df["G"]
    ).rename(columns={"R": "RS"}).drop(columns=["Rank", "G"]),
    
    "Total Sacrifice Bunts": lambda df: df.rename(columns={"SH": "SB"}).assign(
        SBPG=lambda x: x["SB"] / x["G"]
    ).drop(columns=["Rank", "G"]),
    
    "Earned Run Average": lambda df: df.rename(columns={"R": "RA"}).drop(columns=["Rank", "G"]),
    
    "Strikeout-to-Walk Ratio": lambda df: df.rename(columns={"BB": "PBB"})[["Team", "K/BB", "PBB", "SO"]].drop_duplicates(subset="Team"),
    
    "Hits Allowed Per Seven Innings": lambda df: df.rename(columns={"PG": "HAPG"})[["Team", "HA", "HAPG"]],
    
    "Hit Batters": lambda df: df[["Team", "HB"]].drop_duplicates(subset="Team"),

    "WHIP": lambda df: df.assign(BBPG=df['BB']/df['G']).drop(columns=['Rank', 'HA', 'IP']),
}
...
raw_woba_stats = dict(sorted(raw_woba_stats.items()))
...
modeling_stats = softball_stats[['Team', 'HPG',
                'ERA', 'PCT', 
                'OPS', 'BB%',
...
higher_better = ["HPG", "BB%", "PCT", "OPS", 'PYTHAG', 'fWAR', 'oWAR_z', 'pWAR_z', 'K/BB', 'wRC+', 'LOB%', 'wOBA', 'Rank']
...
previous = pd.read_csv(f"./PEAR/PEAR Softball/y{current_season}/straight_up.csv").drop(columns=['Unnamed: 0'])
...
previous.to_csv(f"./PEAR/PEAR Softball/y{current_season}/straight_up.csv")
...
file_path = os.path.join(folder_path, f"Data/softball_{formatted_date}.csv")
...
if now.hour < 24 and now.hour > 7:
    ...
    plt.grid(False)
    plt.savefig(f"./PEAR/PEAR Softball/y{current_season}/Visuals/Ratings_vs_Resume/ratings_vs_resume_{formatted_date}.png", bbox_inches='tight')
    plt.close()
    ...
    plot_logo_bar(
        ...
        filename=f"./PEAR/PEAR Softball/y{current_season}/Visuals/Last_Team_In/last_team_in_{formatted_date}.png",
    ...
    plot_logo_bar(
        ...
        filename=f"./PEAR/PEAR Softball/y{current_season}/Visuals/Last_Host/last_host_{formatted_date}.png",
    ...
    plt.savefig(f"./PEAR/PEAR Softball/y{current_season}/Visuals/Tournament/tournament_{formatted_date}.png", bbox_inches='tight')
    ...
    for line in lines:
        plt.figtext(0.13, y_position, line, ha='left', fontsize=14)
        y_position += vertical_spacing
    plt.savefig(f"./PEAR/PEAR Softball/y{current_season}/Visuals/Projected_Tournament/proj_tournament_{formatted_date}.png", bbox_inches='tight')
    ...
    plt.savefig(f"./PEAR/PEAR Softball/y2025/Visuals/Regional_Win_Prob/regional_win_prob_{formatted_date}.png", bbox_inches='tight')
