...
export default function CbasePage() {
  ...
  const getRatingColor = (value: number, allValues: number[], higherIsBetter: boolean = true) => {
    ...
    if (normalized < 0.25) {
      ...
      // Cyan (#00FFFF) → Light Gray (#D3D3D3)
      const t = (normalized - 0.25) / 0.25;
      const r = Math.round(0 + (211 - 0) * t);
      const g = Math.round(255 + (211 - 255) * t);
      const b = Math.round(255 + (211 - 255) * t);
      return `rgb(${r}, ${g}, ${b})`;
    } else if (normalized < 0.75) {
      // Light Gray (#D3D3D3) → Orange (#FFA500)
      const t = (normalized - 0.5) / 0.25;
      const r = Math.round(211 + (255 - 211) * t);
      const g = Math.round(211 + (165 - 211) * t);
      const b = Math.round(211 + (0 - 211) * t);
      return `rgb(${r}, ${g}, ${b})`;
    } else {
      // Orange (#FFA500) → Dark Red (#8B0000)
