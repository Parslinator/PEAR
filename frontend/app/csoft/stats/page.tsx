...
export default function CsoftStatsPage() {
  ...
  const fetchStats = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/api/softball/stats`);
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = 'softball_advanced_stats.csv';
    a.click();
  ...
  const captureScreenshot = async () => {
    ...
    if (tableElement) {
      try {
        ...
        link.download = `softball_stats_${new Date().toISOString().split('T')[0]}.png`;
                        ...
                        return (
                          <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td className="px-2 py-2 font-semibold text-gray-900 dark:text-white text-sm">
                              ...
                              >
                                <img 
                                  src={`${API_URL}/api/softball-logo/${encodeURIComponent(team.Team)}`}
                                  alt={`${team.Team} logo`}
